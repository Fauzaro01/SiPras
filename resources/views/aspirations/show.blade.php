...
<div class="max-w-4xl mx-auto">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('aspirations.index') }}" class="text-blue-600 hover:text-blue-700 font-medium">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 p-6 text-white">
            <div class="flex justify-between items-start">
                <div class="flex-1">
                    <h1 class="text-2xl font-bold">{{ $aspiration->judul }}</h1>
                    <div class="mt-3 flex flex-wrap gap-3">
                        <span class="bg-white/20 px-3 py-1 rounded-full text-sm">{{ $aspiration->kategori }}</span>
                        <span class="bg-white/20 px-3 py-1 rounded-full text-sm">📍 {{ $aspiration->lokasi }}</span>
                    </div>
                </div>
                <span class="px-4 py-2 rounded-full text-sm font-semibold {{ $aspiration->status_color }}">
        ...
        <div class="p-6 space-y-6">
            ...
            @if(Auth::user()->isAdmin())
                <div class="bg-gray-50 rounded-lg p-4">
                    <h3 class="font-semibold text-gray-800 mb-2">Informasi Pelapor</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        ...
                        @endif
                        <div>
                            <p class="text-gray-600">Tanggal Lapor</p>
            ...
            <!-- Deskripsi -->
            <div>
                <h3 class="font-semibold text-gray-800 mb-3">Deskripsi Masalah</h3>
                <div class="bg-gray-50 rounded-lg p-4">
                    <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ $aspiration->deskripsi }}</p>
                </div>
            </div>
            ...
            @if($aspiration->tanggapan_admin)
                <div>
                    <h3 class="font-semibold text-gray-800 mb-3">Tanggapan Admin</h3>
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ $aspiration->tanggapan_admin }}</p>
                    </div>
                </div>
            @endif
            ...
            @if(Auth::user()->isAdmin())
                <div class="border-t pt-6">
                    <h3 class="font-semibold text-gray-800 mb-4">Update Status & Tanggapan</h3>
                    <form method="POST" action="{{ route('aspirations.update-status', $aspiration) }}" class="space-y-4">
                        ...
                        <div>
                            <label for="tanggapan_admin" class="block text-sm font-medium text-gray-700 mb-2">
                                Tanggapan
                            </label>
                            <textarea 
                        ...
                        <div class="flex justify-end">
                            <button 
                                type="submit"
                                class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-semibold shadow-md hover:shadow-lg transition"
                            >
                                Update Status
                            </button>
                        </div>
            ...
            <!-- Delete Button (for student's own aspiration) -->
            @if(Auth::user()->isSiswa() && $aspiration->user_id === Auth::id() && $aspiration->status === 'pending')
                <div class="border-t pt-6">
        ...
        <div class="space-y-3">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-gray-500">{{ $aspiration->created_at->format('d M Y, H:i') }}</p>
                </div>
            </div>
            @if($aspiration->updated_at != $aspiration->created_at)
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-gray-500">{{ $aspiration->updated_at->format('d M Y, H:i') }}</p>
                    </div>
                </div>
            @endif
        </div>
